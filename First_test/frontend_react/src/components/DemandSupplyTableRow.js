...
class DemandSupplyTableRow extends React.Component {
    constructor(props) {
        super(props);
        this.state = {demandDialogOpen: false, supplyDialogOpen: false, book: {...this.props.item.book, userPrice: this.props.item.price, email: this.props.item.user?.email, info: (this.props.item.info === undefined)? {cover: false, pen: false} : JSON.parse(this.props.item.info)}};
    }
            ...
            <TableRow key={this.props.item.book.id}>
                ...
                {this.props.showAllUsers?
                    <BookInformationDialog owner={!this.props.showAllUsers} book={this.state.book} open={this.state.supplyDialogOpen}
                                    update={this.update} handleClose={this.handeClose}/>
                    :
